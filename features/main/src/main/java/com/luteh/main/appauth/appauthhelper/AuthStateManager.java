...
public class AuthStateManager {
    ...
    public static final String KEY_IS_NOT_FIRST_TIME_AUTHORIZED = "KEY_IS_NOT_FIRST_TIME_AUTHORIZED";
    ...
    private AuthState readState() {
        ...
        try {
            ...
            try {
                Timber.d("145 readState: encrypted currentState-> %s", currentState);
                currentState = new String(Base64.decode(currentState, Base64.NO_WRAP), StandardCharsets.UTF_8);
                Timber.d("145 readState: currentState-> %s", currentState);
    ...
    private void writeState(@Nullable AuthState state) {
        ...
        try {
            ...
            if (state == null) {
                ...
                Timber.d("160 writeState: state-> %s", state.jsonSerializeString());
                String encryptedState = Base64.encodeToString(state.jsonSerializeString().getBytes(StandardCharsets.UTF_8), Base64.NO_WRAP);
                Timber.d("164 writeState: encryptedState-> %s", encryptedState);
//                editor.putString(KEY_STATE, state.jsonSerializeString());
                editor.putString(KEY_STATE, encryptedState);
            }
    ...
    boolean isNotFirstTimeAuthorized() {
        mPrefsLock.lock();
        try {
            return mPrefs.getBoolean(KEY_IS_NOT_FIRST_TIME_AUTHORIZED, false);
        } finally {
            mPrefsLock.unlock();
        }
    }

    void writeIsNotFirstTimeAuthorized() {
        mPrefsLock.lock();
        try {
            SharedPreferences.Editor editor = mPrefs.edit();
            editor.putBoolean(KEY_IS_NOT_FIRST_TIME_AUTHORIZED, true);
