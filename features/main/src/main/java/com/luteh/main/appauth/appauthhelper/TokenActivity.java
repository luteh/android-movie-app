...
public class TokenActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // first time authorization flow has passed, change the state
        mStateManager.writeIsNotFirstTimeAuthorized();
    ...
    private void displayAuthorized() {
        ...
        JWT jwtIdToken = new JWT(idToken);
        long refreshTokenExpiresAt = jwtRefreshToken.getExpiresAt().getTime();
        long idTokenExpiresAt = jwtIdToken.getExpiresAt().getTime();
        Log.d(TAG, "226 displayAuthorized: refreshTokenExpiresAt-> " + refreshTokenExpiresAt);
...
//                : R.string.refresh_token_returned);
        ...
        String refreshTokenTemplate = getResources().getString(R.string.refresh_token_expires_at);
        refreshTokenInfoView.setText(String.format(refreshTokenTemplate,
                DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss ZZ").print(refreshTokenExpiresAt)));

        Timber.d("205 displayAuthorized: refreshToken-> %s", state.getRefreshToken());
...
//                : R.string.id_token_returned);
        String idTokenTemplate = getResources().getString(R.string.id_token_expires_at);
        idTokenInfoView.setText(String.format(idTokenTemplate,
                DateTimeFormat.forPattern("yyyy-MM-dd HH:mm:ss ZZ").print(idTokenExpiresAt)));

        Timber.d("211 displayAuthorized: idToken-> %s", state.getIdToken());
        ...
        if (state.getAccessToken() == null) {
            ...
            if (expiresAt == null) {
                ...
                if (refreshTokenExpiresAt < System.currentTimeMillis()) { // refresh token expired
                    Log.d(TAG, "228 displayAuthorized: refresh token expired, should redirect to login");
                    // TODO: 30/03/2021 Re-launch login
                    // Clear local state first then re-launch login
                    signOut();
                } else {
                    Log.d(TAG, "231 displayAuthorized: refresh token valid, do refreshAccessToken()");
                    refreshAccessToken();
                }
            } else {
...
//        Button refreshTokenButton = (Button) findViewById(R.id.btn_refresh_token);
...
//        refreshTokenButton.setOnClickListener((View view) -> refreshAccessToken());
        ...
        findViewById(R.id.btn_sign_out).setOnClickListener((View view) -> endSession());
