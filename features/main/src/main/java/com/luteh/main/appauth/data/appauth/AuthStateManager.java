...
package com.luteh.main.appauth.data.appauth;
...
public class AuthStateManager {
    ...
    @AnyThread
    public static AuthStateManager getInstance(@androidx.annotation.NonNull Context context) {
    ...
    @AnyThread
    @androidx.annotation.NonNull
    public AuthState replace(@androidx.annotation.NonNull AuthState state) {
        writeState(state);
    ...
    public AuthState updateAfterAuthorization(
            @androidx.annotation.Nullable AuthorizationResponse response,
    ...
    public AuthState updateAfterTokenResponse(
            @androidx.annotation.Nullable TokenResponse response,
            @androidx.annotation.Nullable AuthorizationException ex) {
        AuthState current = getCurrent();
        current.update(response, ex);
        return replace(current);
    ...
    private AuthState readState() {
        ...
        try {
            ...
            try {
                ...
                android.util.Log.w(TAG, "Failed to deserialize stored auth state - discarding");
                return new AuthState();
            }
        } finally {
            mPrefsLock.unlock();
    ...
    @AnyThread
    private void writeState(@androidx.annotation.Nullable AuthState state) {
        mPrefsLock.lock();
        try {
