...
class TrailerActivityTest {
    ...
    @Test
    fun initial_state_trailer_screen() {
        navigate_discover_to_detail_screen()

        with(onView(withId(R.id.cv_watch_trailer))) {
            perform(click())
        }

        onView(withId(R.id.youtube_player)).check(matches(isDisplayed()))
    }

    @Test
    fun back_button_test() {
        initial_state_trailer_screen()
        pressBack()
