...
import com.luteh.movieapp.common.utils.GlideApp
import com.luteh.movieapp.common.utils.MyRequestListener

fun ImageView.setImageFromUrl(imageUrl: String) {
    GlideApp.with(context).load(imageUrl).thumbnail(0.1f).into(this)
}

fun ImageView.setImageFromUrlWithProgressBar(url: String, progress: View, error: View? = null) {
    GlideApp.with(this.context).load(url).thumbnail(0.1f).listener(MyRequestListener(progress, error))
        .into(this)
}
...
fun ImageView.setImageFromResourcesWithProgressBar(resourceId: Int, progress: View) {
    GlideApp.with(this.context).load(resourceId).thumbnail(0.1f)
        .listener(MyRequestListener(progress)).into(this)
}
